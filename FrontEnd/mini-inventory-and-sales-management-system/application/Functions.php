...
function getFileError($errorCode)
{
    if ($errorCode > 0) { // if error code is greater than 0
        ...
        switch ($errorCode) {
            ...
            case 2:
                $msg = "Exceeds Max File Size in html";
                break;
...
function generateRandomCode($codeType, $minLength, $maxLength, $delimiter = "_"){
    ...
    $code = $codeType. $delimiter .  "ewrnlkewn" ;
...
function unsubscribeLink($userEmail, $userId, $userCode, $subsciptionType){
 ...
 function getVatAmount($cumAmount, $vatPercentage){
 ...
 * @param Int $cum_amount the total amount to calculate the discount from
 * @param Int $discount_percentage the percentage of discount
 * @return Float
 */
 function getDiscountAmount($cum_amount, $discount_percentage){
...
function igcEmail(){
    ...
    public function addUser($f_name, $l_name, $email, $password, $role, $mobile1, $mobile2){
