...
class Reservations extends CI_Controller{
    ...
    */
    ...
    public function index(){
        ...
        $data['pageContent'] = $this->load->view('reservations/reservations', $transData, TRUE);
    ...
    */
    ...
    public function latr_(){
        ...
        $totalTransactions = $this->reservations->totalReservations();
        ...
        $config = $this->genlib->setPaginationConfig($totalReservations, "reservations/latr_", $limit, ['onclick'=>'return latr_(this.href);']);
        
        $this->pagination->initialize($config);//initialize the library class
        
        //get all transactions from db
        $data['allReservations'] = $this->reservation->getAll($orderBy, $orderFormat, $start, $limit);
        $data['range'] = $totalTransactions > 0 ? ($start+1) . "-" . ($start + count($data['allReservations'])) . " of " . $totalTransactions : "";
        $data['links'] = $this->pagination->create_links();//page links
        $data['sn'] = $start+1;
        
        $json['resTable'] = $this->load->view('reservations/restable', $data, TRUE);//get view with populated transactions table
    ...
    */
    ...
    public function nso_(){
        ...
        if($allIsWell){//insert each sales order into db, generate receipt and return info to client
            ...
            $json['msg'] = $returnedData ? "reservations successfully processed" : 
            ...
            $this->genmod->addevent("New Reservations", $returnedData['resRef'], $eventDesc, 'reservations', $this->session->admin_id);
       }
    ...
    */
    ...
    private function insertTrToDb($arrOfItemsDetails, $_mop, $_at, $cumAmount, $_cd, $vatAmount, $vatPercentage, $discount_amount, $discount_percentage, $cust_name, $cust_phone, $cust_email){
        ...
        while($this->reservations->isRefExist($ref));
        ...
        foreach($arrOfItemsDetails as $get){
             ...
             */
            $transId = $this->reservations->add($itemName, $itemCode, "", $qtySold, $unitPrice, $totalPrice, $cumAmount, $_at, $_cd, 
        ...
        $this->db->res_complete();//end transaction
        ...
        if($this->db->res_status() === FALSE){
            ...
            $dateInDb = $this->genmod->getTableCol('reservations', 'resDate', 'resId', $transId);
            
            //generate receipt to return
            $dataToReturn['resReceipt'] = $this->genResReceipt($allTransInfo, $cumAmount, $_at, $_cd, $ref, $dateInDb, $_mop, $vatAmount, $vatPercentage, $discount_amount, $discount_percentage, $cust_name, $cust_phone, $cust_email);
    ...
    */
     ...
     * @return type
     */
    private function genResReceipt($allTransInfo, $cumAmount, $_at, $_cd, $ref, $transDate, $_mop, $vatAmount, $vatPercentage, 
        $discount_amount, $discount_percentage, $cust_name, $cust_phone, $cust_email){
        $data['allResInfo'] = $allTransInfo;
        $data['cumAmount'] = $cumAmount;
        $data['amountTendered'] = $_at;
        $data['changeDue'] = $_cd;
        $data['ref'] = $ref;
        $data['resDate'] = $transDate;
        $data['_mop'] = $_mop;
        ...
        $transReceipt = $this->load->view('reservations/resreceipt', $data, TRUE);
    ...
    */
    ...
    public function vtr_(){
        ...
        $transInfo = $this->reservations->getResInfo($ref);
        ...
        if($transInfo){
            ...
            $json['resReceipt'] = $this->genResReceipt($transInfo, $cumAmount, $amountTendered, $changeDue, $ref, 
