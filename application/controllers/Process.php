...
class Process extends CI_Controller{
    ...
    */
    ...
    /**
     * "lilt" = "load processes List Table"
     */
    public function lilt(){
        ...
        $totalItems = $this->db->count_all('processes');
        ...
        $config = $this->genlib->setPaginationConfig($totalItems, "processes/lilt", $limit, ['onclick'=>'return lilt(this.href);']);
        
        $this->pagination->initialize($config);//initialize the library class
        
        //get all items from db
        $data['allProcesses'] = $this->process->getAll($orderBy, $orderFormat, $start, $limit);
        $data['range'] = $totalItems > 0 ? "Showing " . ($start+1) . "-" . ($start + count($data['allProcesses'])) . " of " . $totalItems : "";
        $data['links'] = $this->pagination->create_links();//page links
        $data['sn'] = $start+1;
        $data['cum_total'] = $this->process->getProcessesCumTotal();
        
        $json['processesListTable'] = $this->load->view('processes/processeslisttable', $data, TRUE);//get view with populated items table
    ...
    */
    ...
    public function add(){
        ...
        if($this->form_validation->run() !== FALSE){
             ...
             */
            $insertedId = $this->processes->add(set_value('processesName'), set_value('processesQuantity'), set_value('processesPrice'), 
                    set_value('processesDescription'), set_value('processesCode'));
            
            $itemName = set_value('processName');
            $itemQty = set_value('processQuantity');
            $itemPrice = "&#8369;".number_format(set_value('processPrice'), 2);
            
            //insert into eventlog
            //function header: addevent($event, $eventRowId, $eventDesc, $eventTable, $staffId)
            $desc = "Addition of {$itemQty} quantities of a new process '{$itemName}' with a unit price of {$itemPrice} to stock";
            
            $insertedId ? $this->genmod->addevent("Creation of new process", $insertedId, $desc, "processeses", $this->session->admin_id) : "";
    ...
    */
    ...
    public function gcoandqty(){
        ...
        if($itemCode){
            $item_info = $this->process->getProcessInfo(['code'=>$itemCode], ['quantity', 'unitPrice', 'description']);
    ...
    */
    ...
    public function updatestock(){
        ...
        $this->form_validation->set_rules('_iId', 'Processes ID', ['required', 'trim', 'numeric'], ['required'=>"required"]);
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            $updated = $updateType === "deficit" 
                    ? 
                $this->process->deficit($itemId, $qty, $desc) 
                    : 
                $this->process->newstock($itemId, $qty, $desc);
            ...
            $eventDesc = "<p>{$qty} quantities of {$this->genmod->gettablecol('processes', 'name', 'id', $itemId)} was {$action} stock</p>
                Reason: <p>{$desc}</p>";
            
            //function header: addevent($event, $eventRowId, $eventDesc, $eventTable, $staffId)
            $updated ? $this->genmod->addevent($event, $itemId, $eventDesc, "processes", $this->session->admin_id) : "";
    ...
    */
    ...
    public function edit(){
        ...
        if($this->form_validation->run() !== FALSE){
            $itemId = set_value('_iId');
            $itemDesc = set_value('processDesc');
            $itemPrice = set_value('processPrice');
            $itemName = set_value('processName');
            $itemCode = $this->input->post('processCode', TRUE);
            
            //update item in db
            $updated = $this->process->edit($itemId, $itemName, $itemDesc, $itemPrice);
    ...
    */
    ...
    public function crosscheckName($itemName, $itemId){
        ...
        $itemWithName = $this->genmod->getTableCol('processeses', 'id', 'name', $itemName);
    ...
    */
    ...
    public function crosscheckCode($item_code, $item_id){
        ...
        $item_with_code = $this->genmod->getTableCol('processes', 'id', 'code', $item_code);
    ...
    */
    ...
    public function delete(){
        ...
        if($item_id){
            $this->db->where('id', $item_id)->delete('processes');
